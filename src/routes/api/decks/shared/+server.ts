...
export const POST = (async (event) => {
  ...
  const targetUserPromise = getUserFromEmail(targetUserEmail).then((userData) => Promise.all([userData, getStripeCustomerWithSubscriptions(userData.uid)]));
  const [
    { displayName: sourceUserName = 'A Blend user' },
    [{ displayName: targetUserName }, targetCustomer],
    sourceCustomer,
    deck
  ] = await Promise.all([
  ...
  await sendDeckShareEmail(targetUserEmail, targetUserName, { deckName: deck.name, shareId: sharedKey, sender: sourceUserName })
