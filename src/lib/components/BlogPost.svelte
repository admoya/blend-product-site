...
<article class="blog-post">
  ...
  {#each blogPost.blocks as block}
    {#if block.type === 'heading'}
      {#if block.size === 1}
        <h1 class="my-4">{block.content}</h1>
      {:else if block.size === 2}
        <h2 class="my-4">{block.content}</h2>
      {:else}
        <h3 class="my-4 text-center !font-bold">{block.content}</h3>
      {/if}
    ...
    {:else if block.type === 'image'}
      <div class="image-block" style={block.align ? `padding: 0 1rem; padding-${block.align}: 0; float: ${block.align}` : 'text-align: center;'}>
        <img
          ...
          style={`max-width: 100%; ${!block.align ? 'margin: 0 auto;' : ''}`} />
        {#if block.caption}
